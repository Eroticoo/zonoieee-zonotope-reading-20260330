...
> 由四批清单合并而成，当前版本已覆盖 18 篇论文的英文标题、中文翻译标题、作者、学校与主要贡献点（作者 / 学校待进一步逐篇核对的条目已明确标注）。
...
- **主要贡献点（精确版）**：
  ...
  3. 相比单纯数据驱动或传统滤波方法，该文强调把集合估计与时序学习模型耦合，以兼顾物理约束、噪声鲁棒性与估计精度。
...
- **主要贡献点（精确版）**：
  ...
  3. 将时滞、噪声与故障项统一纳入递推框架，提高了故障检测在有界扰动下的鲁棒性。
...
- **主要贡献点（精确版）**：
  ...
  3. 用 zonotope 近似未来可行轨迹集合或数据驱动后继集合，使 MPC 优化器面对的是可计算的凸集合，而非难以直接处理的原始样本云。
...
- **主要贡献点（精确版）**：
  ...
  3. 该文的重要价值不在控制 / 估计应用，而在于拓展了 zonotope 在组合拓扑与离散几何中的理论边界。
...
- **主要贡献点（精确版）**：
  ...
  3. 该方法的重要意义在于：为后续集合传播、鲁棒估计和控制中的高维 zonotope 提供更轻量但仍保有关键几何信息的表达形式。
...
- **主要贡献点（精确版）**：
  ...
  3. 将 set-membership estimation 与椭球密度思想结合，用于在保守性和估计精度之间取得更优平衡。
...
- **主要贡献点（精确版）**：
  ...
  3. 结合 zonotopic 集合估计与线性规划思想，提高故障检测在参数不确定与有界噪声背景下的鲁棒性。
...
- **主要贡献点（精确版）**：
  ...
  3. 通过 zonotope 递推跟踪触发间隔内的不确定性传播，使容错控制设计具备可计算的鲁棒保证。
...
- **主要贡献点（精确版）**：
  ...
  3. 通过把神经网络输出扩展为区间 / 集合形式，提高故障检测对模型误差与噪声不确定性的适应能力。
...
- **主要贡献点（精确版）**：
  ...
  3. 结合 **semi-infinite programming** 与 **$L_\infty$ 性能指标**，提升非线性系统集员估计的理论严谨性与可计算性。
...
- **主要贡献点（精确版）**：
  ...
  3. 通过 zonotope 对残差与不确定性集合进行可达包络建模，提高攻击检测在有界噪声环境中的鲁棒性。
...
- **主要贡献点（精确版）**：
  ...
  3. 突出“抗离群值”这一特点，说明该方法不仅处理普通噪声，还特别面向异常测量下的鲁棒融合问题。
...
- **主要贡献点（精确版）**：
  ...
  3. 通过 theorem / lemma / property 链条，建立故障检测观测器的设计准则与理论可检测性保证。
...
- **主要贡献点（精确版）**：
  ...
  3. 该方法的重要创新在于：让 zonotope 不仅作用于原始状态空间，也作用于 Koopman 提升空间中的潜在状态表示。
...
- **主要贡献点（精确版）**：
  ...
  3. 提出一种 **switching controller design mechanism**，使攻击下的估计、检测与控制重构形成统一框架。
