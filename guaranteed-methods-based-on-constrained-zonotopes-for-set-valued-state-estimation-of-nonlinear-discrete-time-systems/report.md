...
**系列时间线与本文位置：** 按公开时间，这组 Rego 论文的主线可写成：`2020 年 Automatica 本文 -> 2021 年 Automatica 不变量论文 -> 2022 年 Automatica 联合状态参数估计 -> 2025 年 3 月 31 日 arXiv 多面体松弛论文`。另有一条并行分支是 `2025 年 5 月 23 日在线发表的 Automatica《Line zonotopes》`，它把受约束Zonotope推进到无界与描述符系统。本文是整条主线的起点。

## 1. 研究背景、问题定义与核心思路
### 1.1 研究动机与关键挑战

从 Rego 这组 constrained-zonotope 论文的内部演化看，本文是后续几篇工作的共同出发点。后面的 2021 论文会在这里的 prediction-update 框架上加入 nonlinear invariants，一致性步骤首次成为主角；2022 论文则把这里的状态估计框架扩展到状态-参数联合估计；2025 arXiv 再反过来检视这里依赖均值定理与 Taylor 传播的环节，尝试用多面体松弛替换线性化思路。
