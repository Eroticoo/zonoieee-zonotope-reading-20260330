...
**系列时间线与本文位置：** 这组 Rego 论文的主线顺序是：`2020 年 Automatica 保证性方法 -> 2021 年 Automatica 本文 -> 2022 年 Automatica 联合状态参数估计 -> 2025 年 3 月 31 日 arXiv 多面体松弛方法`。另有一条并行分支是 `2025 年 5 月 23 日在线发表的 Automatica《Line zonotopes》`。本文位于主线第二步，核心任务是在 2020 年框架之上把 nonlinear invariants 系统性并入递推估计。

## 1. 研究背景、问题定义与核心思路
### 1.1 研究动机与关键挑战

如果把 Rego 在 constrained-zonotope 非线性估计上的工作按主线排列，本文正是 2020 Automatica 之后的直接续作。它延续了前文已经建立的 prediction-update 基本框架，但不再满足于只处理传播和测量交集，而是开始把系统本身满足的 nonlinear invariants 也持续注入每一步估计。
